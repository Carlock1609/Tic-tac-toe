...
class Game:
    number_dict = {
                    '10': [0, 0], '20': [0, 1], '30': [0, 2], '40': [0, 3], '50': [0, 4], '60': [0, 5], '70': [0, 6],
    ...
    def calc_winner(self, player, coor_list):
        ...
        for i in range(len(coor_list)):
            ...
            if count == 4:
                self.__repr__()
                print(f"{player.name} You won horizontally")
                exit()
        ...
        for i in range(len(coor_list)):
            ...
            if count == 4:
                self.__repr__()
                print(f"{player.name} You won vertically")
                exit()
            ...
            if diag_ls.__contains__(i+9):
                if diag_ls.__contains__(i+18):
                    if diag_ls.__contains__(i+27):
                        self.__repr__()
                        print(f"{player.name} You won diagonally from lower-left to upper-right.")
                        exit()
            if diag_ls.__contains__(i+11):
                if diag_ls.__contains__(i+22):
                    if diag_ls.__contains__(i+33):
                        self.__repr__()
                        print(f"{player.name} You won diagonally from upper-left to lower-right.")
                        exit()
...
def game_begin(board, p1, p2):
    ...
    player1_numb_ls = []
    player2_numb_ls = []
    print(f"To place {p1.token} or {p2.token} on the grid, enter in an integer for column 1-7 to drop a token.")
